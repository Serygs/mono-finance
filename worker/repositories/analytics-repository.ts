    ...
    const result = await this.database
      .prepare(
        `SELECT transactions.id, transactions.account_id, transactions.direction,
          ...
          COALESCE(categories.id, mapped_categories.id, transactions.original_category_code) AS category_id,
          COALESCE(categories.name, mapped_categories.name, transactions.original_category_name) AS category_name,
