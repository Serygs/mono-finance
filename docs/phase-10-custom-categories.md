...
All endpoints require the authenticated owner and mutation endpoints also require the existing same-origin protection. A referenced category returns `409 category_referenced` on deletion; reset or reassign transaction overrides and imported type mappings first.
