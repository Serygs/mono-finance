...
## 3.1 Custom visual assets

Custom merchant and category icons use the existing D1 database; no R2 bucket,
extra binding, or payment method is required. Files remain private behind
authenticated same-origin Worker URLs, are never committed to Git, and each
fork owns its own mappings. SVG uploads are capped at 64 KB and sanitized to a
static vector subset. PNG, JPEG, WebP, and GIF uploads are byte-signature
validated and capped at 1.9 MB to remain below D1's per-row limit. Visual
priority is merchant custom > category custom > built-in category > merchant
monogram.
